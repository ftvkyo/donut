...
impl GPUCameraData {
    pub fn new(renderer: &Renderer, game: &Game) -> Self {
        let view = game.camera.matrix_view();
        let proj = game.camera.matrix_proj(renderer.aspect_ratio());
    ...
    pub fn update(&self, renderer: &Renderer, game: &Game) {
        ...
        renderer
            .queue
            .write_buffer(&self.view_uniform, 0, bytemuck::cast_slice(&[view]));
        renderer
            .queue
            .write_buffer(&self.proj_uniform, 0, bytemuck::cast_slice(&[proj]));
    }
}
