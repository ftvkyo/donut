<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.10" tiledversion="1.11.2" name="main" tilewidth="16" tileheight="16" tilecount="16" columns="4">
 <image source="main.webp" width="64" height="64"/>
 <tile id="11" probability="0.2"/>
 <tile id="14" probability="0.3"/>
 <tile id="15" probability="0.1"/>
 <wangsets>
  <wangset name="Main" type="corner" tile="-1">
   <wangcolor name="Wall" color="#ff0000" tile="-1" probability="1"/>
   <wangtile tileid="0" wangid="0,0,0,1,0,0,0,0"/>
   <wangtile tileid="1" wangid="0,0,0,0,0,1,0,0"/>
   <wangtile tileid="2" wangid="0,1,0,0,0,1,0,1"/>
   <wangtile tileid="3" wangid="0,1,0,1,0,0,0,1"/>
   <wangtile tileid="4" wangid="0,1,0,0,0,0,0,0"/>
   <wangtile tileid="5" wangid="0,0,0,0,0,0,0,1"/>
   <wangtile tileid="6" wangid="0,0,0,1,0,1,0,1"/>
   <wangtile tileid="7" wangid="0,1,0,1,0,1,0,0"/>
   <wangtile tileid="8" wangid="0,0,0,0,0,1,0,1"/>
   <wangtile tileid="9" wangid="0,1,0,0,0,0,0,1"/>
   <wangtile tileid="10" wangid="0,1,0,1,0,1,0,1"/>
   <wangtile tileid="11" wangid="0,1,0,1,0,1,0,1"/>
   <wangtile tileid="12" wangid="0,0,0,1,0,1,0,0"/>
   <wangtile tileid="13" wangid="0,1,0,1,0,0,0,0"/>
   <wangtile tileid="14" wangid="0,1,0,1,0,1,0,1"/>
   <wangtile tileid="15" wangid="0,1,0,1,0,1,0,1"/>
  </wangset>
 </wangsets>
</tileset>
